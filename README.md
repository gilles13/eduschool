...
`eduschool` est un package R en cours de développement pour consulter des référentiels scolaires français, relier programmes, capacités et notions pédagogiques, produire des exercices de révision reproductibles et visualiser l'organisation des parcours et des données.
...
`remotes` ne doit être installé qu'une seule fois. Pour mettre ensuite `eduschool` à jour, il suffit de relancer :

```r
remotes::install_github("gilles13/eduschool")
```

Il n'est donc pas nécessaire de télécharger manuellement le dépôt GitHub ni de manipuler un fichier ZIP.
...
- des référentiels structurés sur les niveaux, disciplines, programmes et capacités ;
