...
Avant les autres règles, il y en a désormais une qui sert à éviter
qu’`eduschool` ait besoin d’un manuel pour expliquer son manuel :

> **RÈGLE 000 — Simplifier intelligemment, à l’extrême, pour nous rendre
> service.**
>
> **La complexité doit avoir une raison d’exister.**

Une information facultative peut manquer. Un besoin futur peut attendre
le futur. Un cas particulier n’a pas automatiquement droit à sa classe,
sa table, son moteur et son comité de pilotage. Si deux solutions
rendent le même service, la plus simple part avec un avantage assez
sérieux.
...
- **Une nouvelle abstraction n’entre dans `eduschool` que lorsqu’elle a
  trouvé du travail.**
- **Ne pas décider trop tôt est parfois une (bonne) décision.**
...
Les parenthèses de « (bonne) » sont volontaires. Si l’on ne décide pas
encore, comment sait-on déjà que la décision est bonne ? Et décider de
ne pas décider, est-ce déjà décider ? Le lead dev a laissé ici une
petite embuscade logique. Nous aurons un jour à parler de logique.

Une digression n’est d’ailleurs pas nécessairement une sortie du chemin.
Elle peut faire partie du chemin. Une erreur de rendu peut mener à une
question d’architecture, puis à une question sur la décision elle-même.
`eduschool` ne cherche pas à refermer ces portes sous prétexte qu’elles
changent de discipline.
