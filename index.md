...
DuckDB est disponible lorsque les requêtes deviennent plus complexes,
mais il n’est pas nécessaire pour consulter les référentiels, les
rappels ou générer des exercices :
...
Le fichier `launcher.R` reste disponible comme point d’entrée pratique
dans l’arbre source :
...
Le dépôt Git est la référence du projet. Les ZIP ne constituent plus le
mode normal de développement et servent uniquement à transmettre
ponctuellement un instantané.

Les conventions utilisées pour le versionnement sont décrites dans
