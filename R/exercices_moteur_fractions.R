...
generer_addition_fractions = function(niveau_id = "6E", capacite_id = NA_character_, difficulte = 1, seed = NULL) {
  if (!is.null(seed)) set.seed(seed)
  textes = .textes_exercice("fractions", "FRAC_ADD_001")
  ...
  correction = paste0(
    textes[["correction_intro"]], "\n",
    sprintf("(%d x %d + %d x %d)/(%d x %d) = %s.", a, d, c, b, b, d, resultat)
  )
  propositions = c(
    reponse,
    textes[["proposition_denominateurs"]],
    textes[["proposition_multiplier"]],
    sprintf(textes[["proposition_methode"]], personnage)
  )
  feedback = c(
    correction,
    textes[["feedback_denominateurs"]],
    textes[["feedback_multiplier"]],
    sprintf(textes[["feedback_methode"]], resultat, personnage)
  )
...
generer_multiplication_fractions = function(niveau_id = "5E", capacite_id = NA_character_, difficulte = 1, seed = NULL) {
  if (!is.null(seed)) set.seed(seed)
  textes = .textes_exercice("fractions", "FRAC_MULT_001")
  ...
  enonce = sprintf(textes[["enonce"]], a, b, c, d)
  correction = paste0(textes[["correction_intro"]], "\n", sprintf("%s = %s.", reponse, resultat))
  propositions = c(
    reponse,
  ...
  feedback = c(
    ...
    textes[["feedback_croisement"]],
    textes[["feedback_numerateur"]]
  )
  qcm = .qcm_fraction(
...
generer_division_fractions = function(niveau_id = "5E", capacite_id = NA_character_, difficulte = 1, seed = NULL) {
  if (!is.null(seed)) set.seed(seed)
  textes = .textes_exercice("fractions", "FRAC_DIV_001")
  b = sample(c(2L, 3L, 4L, 5L, 6L, 8L), 1L)
  d = sample(c(2L, 3L, 4L, 5L, 6L, 8L), 1L)
  a = sample(seq_len(b - 1L), 1L)
  c = sample(seq_len(d - 1L), 1L)
  ...
  correction = paste0(
    sprintf(textes[["correction_intro"]], c, d, d, c), "\n",
    textes[["correction_suite"]], "\n",
    sprintf("%s = %s.", reponse, quotient)
  )
  ...
  feedback = c(
    correction,
    sprintf(textes[["feedback_inverser"]], c, d, d, c),
    sprintf(textes[["feedback_premiere"]], a, b),
    sprintf(textes[["feedback_seconde"]], c, d)
  )
  qcm = .qcm_fraction(
...
generer_fraction_quantite = function(niveau_id = "6E", capacite_id = NA_character_, difficulte = 1, seed = NULL) {
  if (!is.null(seed)) set.seed(seed)
  textes = .textes_exercice("fractions", "FRAC_QTE_001")
  den = sample(c(2,3,4,5,6,8,10),1)
  num = sample(seq_len(den-1),1)
  base = sample(2:12,1) * den
  rep = base * num / den

  enonce = sprintf(textes[["enonce"]], base, num, den)
...
generer_fraction_quotient = function(niveau_id = "6E", capacite_id = NA_character_, difficulte = 1, seed = NULL) {
  ...
  if (cas == "fraction_vers_quotient") {
    ...
    enonce = sprintf(textes[["enonce_quotient_vers_fraction"]], num, den)
  ...
  correction = sprintf(textes[["correction"]], num, den, num, den)
  feedback = c(
    correction,
    textes[["feedback_inverse"]],
    textes[["feedback_pas_quotient"]],
    textes[["feedback_pas_quotient"]]
  )
  qcm = .qcm_fraction(
...
generer_fraction_droite = function(niveau_id = "6E", capacite_id = NA_character_, difficulte = 1, seed = NULL) {
  if (!is.null(seed)) set.seed(seed)
  textes = .textes_exercice("fractions", "FRAC_DROITE_001")
  ...
  correction = sprintf(textes[["correction"]], den, k, reponse)
  propositions = c(
    reponse,
  ...
  feedback = c(
    correction,
    textes[["feedback_inverse"]],
    textes[["feedback_moins"]],
    textes[["feedback_partage"]]
  )
  qcm = .qcm_fraction(
...
generer_fraction_equivalente = function(niveau_id = "6E", capacite_id = NA_character_, difficulte = 1, seed = NULL) {
  if (!is.null(seed)) set.seed(seed)
  textes = .textes_exercice("fractions", "FRAC_EQUIV_001")
  den = sample(3:9, 1L)
  num = sample(seq_len(den - 1L), 1L)
  facteur = sample(2:5, 1L)
  reponse = .fraction_brute(num * facteur, den * facteur)
  enonce = sprintf(textes[["enonce"]], num, den)
  propositions = c(
    reponse,
...
generer_fraction_comparer = function(niveau_id = "6E", capacite_id = NA_character_, difficulte = 1, seed = NULL) {
  if (!is.null(seed)) set.seed(seed)
  textes = .textes_exercice("fractions", "FRAC_COMP_001")
  ...
  correction = if (cas == "meme_denominateur") {
    sprintf(textes[["correction_meme_den"]], a, c, a, b, signe, c, d)
  } else {
    sprintf(textes[["correction_meme_num"]], a, b, signe, c, d)
  }
  propositions = c(reponse, if (signe == "<") ">" else "<", "=", textes[["proposition_impossible"]])
  feedback = c(
    correction,
    textes[["feedback_inverse"]],
    textes[["feedback_egalite"]],
    textes[["feedback_suffisant"]]
  )
  qcm = .qcm_fraction(
...
generer_fraction_encadrer = function(niveau_id = "6E", capacite_id = NA_character_, difficulte = 1, seed = NULL) {
  if (!is.null(seed)) set.seed(seed)
  textes = .textes_exercice("fractions", "FRAC_ENCADR_001")
  den = sample(2:8, 1L)
  ...
  enonce = sprintf(textes[["enonce"]], frac)
  propositions = c(
    reponse,
  ...
  correction = sprintf(textes[["correction"]], num, entier, den, reste, frac, entier, entier + 1L)
  feedback = c(
    correction,
    textes[["feedback_bas"]],
    textes[["feedback_haut"]],
    textes[["feedback_superieure"]]
  )
  qcm = .qcm_fraction(
...
generer_soustraction_fractions = function(niveau_id = "6E", capacite_id = NA_character_, difficulte = 1, seed = NULL) {
  if (!is.null(seed)) set.seed(seed)
  textes = .textes_exercice("fractions", "FRAC_SUB_001")
  den = sample(c(3L, 4L, 5L, 6L, 8L, 10L, 12L), 1L)
  nums = sort(sample(seq_len(den - 1L), 2L), decreasing = TRUE)
  a = nums[[1L]]; c = nums[[2L]]
  reponse = fmt_fraction(a - c, den)
  enonce = sprintf(textes[["enonce"]], a, den, c, den)
  correction = sprintf(textes[["correction"]], a, c, den, reponse)
  ...
  feedback = c(
    correction,
    sprintf(textes[["feedback_faux"]], reponse),
    sprintf(textes[["feedback_faux"]], reponse),
    sprintf(textes[["feedback_faux"]], reponse)
  )
  qcm = .qcm_fraction(
...
generer_fraction_terme_manquant = function(niveau_id = "6E", capacite_id = NA_character_, difficulte = 1, seed = NULL) {
  if (!is.null(seed)) set.seed(seed)
  textes = .textes_exercice("fractions", "FRAC_MANQ_001")
  ...
  enonce = sprintf(textes[["enonce"]], connu, den, cible, den)
  correction = sprintf(textes[["correction"]], connu, cible, cible, connu, manque, reponse)
  ...
  feedback = c(
    correction,
    textes[["feedback_addition"]],
    textes[["feedback_denominateur"]],
    textes[["feedback_recopie"]]
  )
  qcm = .qcm_fraction(
...
generer_fraction_par_entier = function(niveau_id = "6E", capacite_id = NA_character_, difficulte = 1, seed = NULL) {
  if (!is.null(seed)) set.seed(seed)
  textes = .textes_exercice("fractions", "FRAC_MULT_ENT_001")
  den = sample(c(2L, 3L, 4L, 5L, 6L, 8L), 1L)
  num = sample(seq_len(den - 1L), 1L)
  entier = sample(2:8, 1L)
  reponse = fmt_fraction(entier * num, den)
  enonce = sprintf(textes[["enonce"]], entier, num, den)
  correction = sprintf(textes[["correction"]], entier, num, den, entier, num, den, reponse)
  propositions = c(
    reponse,
...
.varier_forme_fraction = function(exercice) {
  ...
  textes = .textes_exercice("fractions", id)
  ...
  if (id == "FRAC_QTE_001" && as.integer(exercice$seed) %% 4L == 0L) {
    ...
    return(.nommer_notion_exercice(
      exercice,
      notion = textes[["var_notion_distributivite"]],
      formule = formule,
      explication = textes[["var_explication_distributivite"]]
    ))
  }
  ...
  construire = function(enonce, reponse, correction, distracteurs, forme_question, feedback_faux = NULL) {
    ...
    if (is.null(feedback_faux)) {
      feedback_faux = vapply(distracteurs, function(x) {
        sprintf(textes[["var_feedback_relation"]], x)
      }, character(1))
    }
    feedback = c(correction, feedback_faux)
  ...
  if (id == "FRAC_ADD_001") {
    ...
    if (forme == "boite_a_trou") {
      ...
      enonce = sprintf(textes[["var_enonce_trou"]], c, d, resultat)
      correction = sprintf(textes[["var_correction_trou"]], a, b, a, b, c, d, resultat)
      faux = unique(c(.fraction_brute(c,d), .fraction_brute(a+c,b), .fraction_brute(a,b+d), .fraction_brute(b,a)))
    } else {
      reponse = resultat
      enonce = sprintf(textes[["var_enonce_inverse"]], a,b,c,d)
    ...
    if (forme == "boite_a_trou") {
      enonce = sprintf(textes[["var_enonce_trou"]], a, den, exercice$reponse)
      correction = sprintf(textes[["var_correction_trou"]], c,den,a,den,c,den,exercice$reponse)
      faux = unique(c(.fraction_brute(a,den), .fraction_brute(a-c,den), .fraction_brute(c+1L,den), .fraction_brute(max(1L,c-1L),den)))
    } else {
      enonce = sprintf(textes[["var_enonce_inverse"]], a,den,exercice$reponse)
      correction = sprintf(textes[["var_correction_inverse"]], c,den)
    ...
    if (forme == "boite_a_trou") {
      ...
      enonce = sprintf(textes[["var_enonce_inverse"]], c,d,resultat)
    }
    correction = sprintf(textes[["var_correction"]], a,b,a,b,c,d,resultat)
    ...
    reponse = .fraction_brute(a,b)
    if (forme == "boite_a_trou") {
      enonce = sprintf(textes[["var_enonce_trou"]], c,d,quotient)
    } else {
      enonce = sprintf(textes[["var_enonce_inverse"]], c,d,quotient)
    }
    correction = sprintf(textes[["var_correction"]], a,b,a,b,c,d,quotient)
    ...
    if (forme == "boite_a_trou") {
      ...
      enonce = sprintf(textes[["var_enonce_trou"]], num,den,utilise)
      correction = sprintf(
        textes[["var_correction_trou"]],
      ...
      enonce = sprintf(
        textes[["var_enonce_inverse"]],
        contexte[[1L]], base, contexte[[2L]], num, den
      )
      correction = sprintf(
        textes[["var_correction_inverse"]],
