---
title: "Maths & économie — Un prix est-il toujours proportionnel à ce qu'on achète ?"
author: "eduschool"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Maths & économie — Un prix est-il toujours proportionnel à ce qu'on achète ?}
...
tarifs = data.frame(
  quantite = rep(0:5, 2),
  tarif = rep(c("Tarif A : 2q", "Tarif B : 4 + 2q"), each = 6)
)
...
ggplot(
  ...
  labs(
    title = "Deux prix qui augmentent... mais pas de la même manière",
    subtitle = "Observe surtout ce qui se passe lorsque la quantité vaut 0.",
    x = "Quantité",
    y = "Prix (euros)",
    linetype = NULL,
    caption = "Données fictives construites pour comprendre le mécanisme."
  ) +
...
```{r donnees-electricite}
electricite = data.frame(
  annee = 1990:2025,
  indice = c(
    71.10, 73.10, 74.80, 75.70, 76.20, 76.90,
    77.70, 76.00, 74.30, 71.00, 69.50, 69.20,
    69.80, 70.60, 71.60, 71.50, 72.00, 73.00,
    74.01, 75.39, 77.19, 82.20, 84.75, 90.28,
    95.41, 100.00, 103.12, 103.55, 104.91, 109.00,
    115.47, 118.90, 127.66, 144.13, 166.78, 146.92
  )
)
```

```{r graphique-electricite}
ggplot(
  electricite,
  ...
  geom_point(
    data = subset(
      electricite,
      annee %in% c(1990, 2015, 2024, 2025)
    ),
    size = 2.5
  ) +
  geom_hline(
  ...
  labs(
    title = "Le prix de l'électricité vu sur une longue période",
    subtitle = "Indice annuel des prix à la consommation — base 2015",
    x = NULL,
    y = "Indice",
    caption = paste(
      "Source : Insee, IPC, électricité, ensemble des ménages, France,",
      "base 2015, série 001764662."
    )
  ) +
  theme_minimal(base_size = 12) +
  theme(
